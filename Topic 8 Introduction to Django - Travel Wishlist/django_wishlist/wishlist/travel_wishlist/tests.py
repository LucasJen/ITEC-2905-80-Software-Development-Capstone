...
class VisitedList(TestCase):
    ...
    def test_visited_list_shows_visited_pplaces(self):
        response = self.client.get(reverse('places_visited'))
        self.assertTemplateUsed(response, 'travel_wishlist/visited.html')
...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        response = self.client.post(add_place_url, new_place_data, follow= True) # follow allows the test to follow redirects, not enabled by default

        self.assertTemplateUsed(response, 'travel_wishlist/wishlist.html')

        response_places = response.context['places']
        self.assertEqual(1, len(response_places)) # checks there is only one place
    ...
    class TestVisitPlace(TestCase):
        ...
        def test_visit_place(self):
            visit_place_url = reverse('place_was_visited', arg=(2, ))
        ...
        def test_non_existent_place(self):
            visit_nonexistent_place_url = reverse('place_was_visited', args=(12235, ))
